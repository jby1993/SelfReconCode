...
class SMPL(nn.Module):
	def __init__(self, model_path, joint_type = 'cocoplus', obj_saveable = False):
		...
		if os.path.isfile(model_path+'.pkl'):
			with open(model_path+'.pkl', 'rb') as reader:
				model = pickle.load(reader)
		elif os.path.isfile(model_path+'.txt'):
			with open(model_path+'.txt', 'r') as reader:
	...
	def forward(self, beta, theta, get_skin = False, theta_in_rodrigues=True):
		...
		self.J = torch.stack([Jx, Jy, Jz], dim = 2)
		...
		self.J_transformed, self.A = batch_global_rigid_transformation(Rs, self.J, self.parents, rotate_base = False)

		# weight = self.weight[:num_batch]        
		# W = weight.view(num_batch, -1, 24)
		W=self.weight.expand(num_batch,*self.weight.shape[1:])
		T = torch.matmul(W, self.A.view(num_batch, 24, 16)).view(num_batch, -1, 4, 4)
		...
		if get_skin:
			...
			return joints
	def avatar(self,Tvs,beta,theta,theta_in_rodrigues=True):
		if not self.cur_device:
			device = Tvs.device
			self.cur_device = torch.device(device.type, device.index)
		...
		if theta_in_rodrigues:
			...
			Rs = theta.view(-1,24,3,3)
		v_posed = Tvs.unsqueeze(0).expand(num_batch,-1,3)
...
def getSMPL(gender):
	return SMPL(os.path.join(os.path.dirname(__file__),'model/%s_smpl_with_cocoplus_reg'%gender), obj_saveable = True)
def getTmpFile():
