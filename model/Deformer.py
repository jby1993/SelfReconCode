...
class CompositeDeformerTmpWs(nn.Module):
	def __init__(self,deformers):
		super().__init__()
		self.N=len(deformers)
		self.defs=nn.ModuleList(deformers)
	...
	def forward(self,ps,conds,batch_inds=None,**kwargs):
		...
		dps=self.defs[0](ps,conds[0],batch_inds,**kwargs)
		out=self.defs[1]([ps,dps],conds[1],batch_inds,**kwargs)
		return out
...
class RigidTransformer(nn.Module):
	...
	def forward(self,ps,conds,batch_inds=None,**kwargs):
		...
		if batch_inds is not None:
			x=Rs[batch_inds]@ps.view(-1,3,1)+Ts[batch_inds].view(-1,3,1)
			x=x.reshape(-1,3)
		else:
			x=Rs.view(-1,1,3,3)@ps.unsqueeze(-1)+Ts.view(-1,1,3,1)
			x=x.squeeze(-1)
		return x 
...
class MLPCondTranslator(nn.Module):
	def __init__(self,feature_vector_size,frame_vector_size,multires,weight_norm=False):
		super().__init__()
		self.enableSdfcond=True
		self.featureTrans=FeatureTransformer(feature_vector_size,frame_vector_size,True)
		dims = [3+feature_vector_size,512,512,512,512,3]
		self.feature_vector_size=feature_vector_size
		self.frame_vector_size=frame_vector_size
		self.embed_fn = None
		self.multires=multires
		if multires > 0:
			embed_fn, input_ch = get_embedder(multires)
	...
	def forward(self,ps,conds,batch_inds=None,**kwargs):
		...
		if self.embed_fn is not None:
			if ratio is None:	#one weight, all equal one
				...
				ps = self.embed_fn(ps, utils.annealing_weights(self.multires,ratio))
		frameconds,sdfconds=conds
		if batch_inds is not None:
			conds=self.featureTrans(sdfconds,frameconds[batch_inds])
			x=torch.cat([ps,conds],dim=1)
		else:
			conds=self.featureTrans(sdfconds.reshape(-1,self.feature_vector_size),frameconds.view(-1,1,self.frame_vector_size).expand(-1,ps.shape[1],self.frame_vector_size).reshape(-1,self.frame_vector_size))
			x=torch.cat([ps,conds.reshape(-1,ps.shape[1],self.feature_vector_size)],dim=-1).reshape(-1,ps.shape[-1]+self.feature_vector_size)
...
class NerfyTranslator(nn.Module):
	def __init__(self,feature_vector_size,multires):
		super().__init__()
		dims = [3+feature_vector_size,512,512,512,512,9]
		self.feature_vector_size=feature_vector_size
		self.embed_fn = None
		self.multires=multires
		if multires > 0:
	...
	def forward(self,ps,conds,batch_inds=None,**kwargs):
		...
		for l in range(0, self.num_layers - 1):
			...
			if l < self.num_layers - 2:
				x = self.relu(x)
		if x.shape[0]!=ps.shape[0]:
			x=x.view(ps.shape[0],-1,9)

		v=x[...,:3]
		s=x[...,3:6]
		t=x[...,6:9]
		theta=v.norm(dim=-1,keepdim=True)
		q=torch.cat([torch.cos(theta),torch.sin(theta)*(v/theta)],dim=-1)
		R=utils.quat2mat(q.reshape(-1,4)).reshape(-1,3,3)
		if R.shape[0]!=v.shape[0]:
			R=R.reshape(v.shape[0],-1,3,3)

		x=R.matmul((ps[...,:3]-s).unsqueeze(-1)).squeeze(-1)+s+t
		self.offset=x-ps[...,:3]
		return x

# from modules import FCBlock
# class SIRENTranslator(nn.Module):
# 	def __init__(self,feature_vector_size):
# 		super().__init__()
# 		self.feature_vector_size=feature_vector_size
# 		self.siren=FCBlock(3+feature_vector_size,3,3,512,True,'sine')
# 	def forward(self,ps,conds,batch_inds=None,**kwargs):
# 		if batch_inds is not None:
# 			x=torch.cat([ps,conds[batch_inds]],dim=1)
# 		else:
...
def getTranslatorNet(device,conf):
	if 'type' in conf:
		if conf.get_string('type')=='MLPCondTranslator':
			return MLPCondTranslator(conf.get_int('sdfcondlen'),conf.get_int('condlen'),multires=conf.get_int('multires')).to(device)
		else:
