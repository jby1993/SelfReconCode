...
torch::Tensor interp2x_boundary2d_backward(
    const torch::Tensor& grad_output) {
      CHECK_INPUT(grad_output);

      return interp2x_boundary2d_cuda_backward(grad_output);
}


PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &interp2x_boundary2d_forward, "forward (CUDA)");
  m.def("backward", &interp2x_boundary2d_backward, "backward (CUDA)");
}